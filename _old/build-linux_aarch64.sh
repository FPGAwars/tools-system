#######################################################################
# libusb and libftdi builder for Ubuntu tablet (linux_aarch64)
# (C) BQ. March-2016
...
sudo apt-get install libtool autoconf gcc-aarch64-linux-gnu \
                     g++-aarch64-linux-gnu
...
if [ $COMPILE_LIBUSB == "1" ]; then
    ...
    if [ $COMPILE_LISTDEVS == "1" ]; then
        ...
        $HOST-gcc -o listdevs listdevs.c -I $WORK/$BUILD_DIR/include/libusb-1.0/  \
                  -L $WORK/$BUILD_DIR/lib  -L /usr/aarch64-linux-gnu/lib/ \
...
cp $WORK/$BUILD_DATA/$ARCH/toolchain-$ARCH.cmake.libftdi \
   $WORK/$BUILD_DIR/$LIBFTDI_FILENAME/toolchain-$ARCH.cmake
