...
if [ $COMPILE_LIBUSB == "1" ]; then
    ...
    # No udev used
    ./configure --prefix=$PREFIX USE_UDEV=0  --host=$HOST "CFLAGS=-m32" \
    ...
    if [ $COMPILE_LISTDEVS == "1" ]; then
        ...
        cd examples
        gcc -m32 -o listdevs listdevs.c -I $WORK/$BUILD_DIR/include/libusb-1.0/  \
...
if [ $COMPILE_LIBFTDI == "1" ]; then
    ...
    if [ $COMPILE_FIND_ALL == "1" ]; then
        ...
        cd ../examples
        gcc -m32 -o find_all find_all.c -I $PREFIX/include/libftdi1/ \
