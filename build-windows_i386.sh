...
sudo apt-get install libtool autoconf gcc-multilib g++-multilib
...
if [ $COMPILE_LIBUSB == "1" ]; then
    ...
    if [ $COMPILE_LISTDEVS == "1" ]; then
        ...
        cd examples
        $HOST-gcc -o listdevs.exe listdevs.c -I $WORK/$BUILD_DIR/include/libusb-1.0/  \
               -L $WORK/$BUILD_DIR/lib  -static -lusb-1.0

        # -- Copy the executable into the packages/bin dir
        cp listdevs.exe $WORK/$PACK_DIR/$BUILD_DIR/bin
    fi
fi
...
cp $WORK/$BUILD_DATA/$ARCH/toolchain-mingw32.cmake.libftdi \
   $WORK/$BUILD_DIR/$LIBFTDI_FILENAME/toolchain-mingw32.cmake
...
if [ $COMPILE_LIBFTDI == "1" ]; then
    ...
    if [ $COMPILE_FIND_ALL == "1" ]; then
        ...
        cd ../examples
        $HOST-gcc -o find_all.exe find_all.c -I $PREFIX/include/libftdi1/ \
            -L $PREFIX/lib -static -lftdi1 -lusb-1.0

        # -- Copy the executable into the packages/bin dir
        cp find_all.exe $WORK/$PACK_DIR/$BUILD_DIR/bin
    fi
fi
